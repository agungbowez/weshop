<?php
	// start protek halaman login dan halaman menu dari user yang mengakses langsung melalui url
	
	if($user_id){
		$module = isset ($_GET['module']) ? $_GET['module'] : false;
		$action = isset($_GET['action']) ? $_GET['action'] : false;
		$mode = isset($_GET['mode']) ? $_GET['action'] : false;
...
<div id="bg-page-profile">
	...
	<div id="profile-content">
		...
		*/
		$file = "$module,$action,$mode";
		if(file_exists($file)){
			include_once($file);
		}else{
			echo "<h3> maaf, halaman tidak ditemukan";
		}
